const getApiBase = () => {
  if (window.location.protocol === 'file:') {
    return 'http://localhost/marketplace_v2/api/routes';
  }
  let pathname = window.location.pathname.replace(/\\/g, '/');
  let idx = pathname.indexOf('/frontend/');
  if (idx !== -1) {
    let basePath = pathname.substring(0, idx);
    return window.location.origin + basePath + '/api/routes';
  }
  return window.location.origin + '/api/routes';
};
...
const getImgUrl = (path, depth = 0) => {
  ...
  if (window.location.protocol === 'file:') {
    ...
    let calcDepth = depth;
    if (depth === 0) {
      if (pathname.includes('/frontend/panel/') || pathname.includes('/frontend/admin/')) {
        calcDepth = 2;
