...
const API = { token:()=>localStorage.getItem('token'), user:()=>{try{return JSON.parse(localStorage.getItem('user'))}catch{return null}}, headers:(a=false)=>{let h={'Content-Type':'application/json'}; if(a)h.Authorization='Bearer '+localStorage.getItem('token'); return h;}, async req(p,o={}){let url = window.AppConfig && typeof window.AppConfig.resolveApiUrl === 'function' ? window.AppConfig.resolveApiUrl(p) : (API_BASE+p); let r=await fetch(url,o); if(r.status===401){localStorage.removeItem('token'); localStorage.removeItem('user'); window.location.href = window.AppConfig && typeof window.AppConfig.resolveUrl === 'function' ? window.AppConfig.resolveUrl('login.html') : (window.location.pathname.includes('/admin/') || window.location.pathname.includes('/panel/') ? '../login.html' : 'login.html'); return {success:false,message:'Sesión expirada. Redirigiendo...',status:401};} let t=await r.text(); try{return JSON.parse(t)}catch{return {success:false,message:'La API no devolvió JSON válido',raw:t,status:r.status}}}, get(p,a=false){return this.req(p,{headers:this.headers(a)})}, post(p,d,a=false){return this.req(p,{method:'POST',headers:this.headers(a),body:JSON.stringify(d)})}, put(p,d){return this.req(p,{method:'PUT',headers:this.headers(true),body:JSON.stringify(d)})}, del(p){return this.req(p,{method:'DELETE',headers:this.headers(true)})} };
...
const getImgUrl = (path, size = 'full') => {
  if (window.AppConfig && typeof window.AppConfig.resolveImageUrl === 'function') {
    return window.AppConfig.resolveImageUrl(path, size);
  }
